...
export default function Shop({ isOpen, onClose, userId, onBuy }){
  ...
  const handleBuy = async (ing) => {
    if (coins < ing.price) { alert('Pas assez de pièces!'); return }
    try {
      const r = await fetch('http://localhost:4000/shop/buy', {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({ userId, ingredientKey: ing.key })
      })
      const data = await r.json()
      if (data.success) {
        setCoins(data.coins)
        onBuy && onBuy(ing.key) // notify parent
      } else {
          ...
          {ingredients.map(ing=> (
            <div key={ing.key} style={{border:'1px solid #ddd',padding:12,borderRadius:6,display:'flex',flexDirection:'column',alignItems:'center',gap:8}}>
              ...
              <div style={{textAlign:'center'}}>
                ...
                <div style={{fontSize:12,color:'#666'}}>Prix: {ing.price}</div>
              </div>
              <button onClick={()=>handleBuy(ing)} style={{width:'100%',padding:'6px 12px',background:coins >= ing.price ? '#4CAF50' : '#ccc',color:'#fff',border:'none',borderRadius:4,cursor: coins >= ing.price ? 'pointer' : 'not-allowed'}}>
                Acheter
              </button>
            </div>
          ))}
